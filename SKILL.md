...
Before writing `phase6.md`, draft the convergence adjudication.
Give one separate checker per iteration the frozen ledger, dispositions,
objective types, convergence conditions, and draft, but no subject access or
research. It returns only `conformant`, `non-conformant` with the violated
condition and existing IDs, or `indeterminate` with a reason; it cannot introduce
a finding, re-derive materiality, or change the outcome.

A `non-conformant` result permits one corrected draft and final response. It may
repair representation or rule application only; an evidence-bound disposition
or outcome change needs new validation. A substantive post-`conformant` change
invalidates conformance; it, a second `non-conformant`, or any `indeterminate`
result precludes full convergence for the iteration and becomes a standing
bounded-convergence condition. Without a separate checker, record that and cap
the iteration at bounded convergence instead of self-checking. A correction
is not a material increase in validated knowledge for recursive-loop
termination, and a conformance failure cannot be waived through accepted risk.
Record the results, final decision, and conformance in `phase6.md`. Using the
strongest available identity, bind conformance to the frozen ledger,
objective/constraint mappings, dispositions, conditions, and exact checked
adjudication.
